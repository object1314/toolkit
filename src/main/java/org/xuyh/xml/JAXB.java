...
public class JAXB {
	...
	public void encode(OutputStream out, JAXBBean bean) throws Exception {
		if (null == context)
			synchronized (this) {
				if (null == context)
					context = JAXBContext.newInstance(JAXBBean.class);
			}
		Marshaller marshaller = context.createMarshaller();
		marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
		marshaller.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
		marshaller.setProperty(Marshaller.JAXB_FRAGMENT, Boolean.FALSE);
		if (null == outputFactory)
			synchronized (this) {
				if (null == outputFactory)
					outputFactory = XMLOutputFactory.newInstance();
			}
		JAXBStreamWriter writer = new JAXBStreamWriter(outputFactory.createXMLStreamWriter(out, "UTF-8"));
	...
	public JAXBBean decode(InputStream in) throws Exception {
		...
		JAXBStreamReader reader = new JAXBStreamReader(inputFactory.createXMLStreamReader(in, "UTF-8"));
