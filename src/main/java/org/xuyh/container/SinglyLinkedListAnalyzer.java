...
public class SinglyLinkedListAnalyzer {
	...
	public static <T> SinglyLinkedNode<T> getIntersection(SinglyLinkedNode<T> ahead, SinglyLinkedNode<T> bhead) {
		if (isNilNode(ahead) || isNilNode(bhead))
			return null;
		...
		// Set a as a tail when a list is not a circular one
		if (null == a)
			for (a = ahead; !isNilNode(a.getNext()); a = a.getNext())
				;
